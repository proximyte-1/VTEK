...
const ViewOriginContract = () => {
  ...
  const fecthDataMesin = async () => {
    try {
      const response = await axios.get(
        `${import.meta.env.VITE_API_URL}api/get-contract-machine-origin`,
        {
          params: {
            id_contract: id,
          },
  ...
  return (
    <Paper sx={{ padding: 3, marginBottom: 5 }} elevation={4}>
      <Typography variant="h5" marginBottom={"1.5em"} gutterBottom>
        View Kontrak Awal - {data.no_contract}
      </Typography>
...
export default ViewOriginContract;

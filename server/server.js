...
const navFilterTopNoSeriEncode = (f_no_seri, v_no_seri, f_no_cus, v_no_cus) => {
  const data = encodeURIComponent(
    `${f_no_seri} eq '${v_no_seri}' and ${f_no_cus} eq '${v_no_cus}'`
  );
  return `?$filter=${data}&$top=1`;
};
...
app.get("/api/get-area-lk", async (req, res) => {
  ...
  try {
    const result = await pool.query(
      `SELECT area.id as id_area, area.id_teknisi, cus.no_cus, area.id, area.kode_area, area.groups, area.id_supervisor 
...
app.get("/api/get-approver-status", async (req, res) => {
  const { id_appr } = req.query;

  try {
    const fetchApprover = await pool.query(
      `SELECT users.name, appr.approved, users.id as id_user, appr.id FROM dbo.${process.env.TABLE_LK_APPR} AS appr JOIN dbo.${process.env.TABLE_USER} AS users ON appr.id_approver = users.id WHERE appr.id_lk = '${id_appr}'`
    );
    const data = fetchApprover.recordset;
...
app.get("/api/lk-approve", async (req, res) => {
  const { id_approval, id } = req.query;
  ...
  try {
    ...
    const fetchApprover = await pool.query(
      `UPDATE dbo.${process.env.TABLE_LK_APPR} SET approved = '1' WHERE id = '${id_approval}'`
    );

    const data = fetchApprover.recordset;

    const checkStatus = await pool.query(
      `SELECT count(*) as count FROM dbo.${process.env.TABLE_LK_APPR} WHERE id_lk = '${id}' AND approved = '2'`
    );

    const count_pending = checkStatus?.recordset[0]?.count;

    if (count_pending === 0) {
      const update_status_lK = await pool.query(
        `UPDATE dbo.${process.env.TABLE_LK} SET status_appr = '1' WHERE id = '${id}'`
      );

      const res_update_lk = update_status_lK.recordset;
    }
...
app.get("/api/nav-one-by-no-cus", (req, res) => {
  const no_cus = req.query.no_cus;
  const navURL =
    process.env.NAV_WS_URL + navFilterEncode("Sell_to_Customer_No", no_cus);
...
app.get("/api/nav-data-noseri-name", (req, res) => {
  const { no_cus, no_seri } = req.query;

  const navURL =
    process.env.NAV_WS_URL +
    navFilterTopNoSeriEncode(
      "Serial_No",
      no_seri,
      "Sell_to_Customer_No",
      no_cus
  ...
  try {
    ...
    const query = `
      INSERT INTO dbo.${process.env.TABLE_LK} (
        ...
        '${created_by}', '${filePath}', '${no_lap}', '${no_fd}', '${id_teknisi}', '${status_appr}'
      )
    `;
...
app.get("/api/get-brg", async (req, res) => {
...
app.post("/api/create-approval", async (req, res) => {
  const { id_lk, id_area, approved } = req.body;
  ...
  try {
    ...
    const query_area = `SELECT * FROM dbo.${process.env.TABLE_AREA} WHERE id = '${id_area}'`;

    const exec_area = await request.query(query_area);

    const res_area = exec_area.recordset[0];
    const arr_approver = JSON.parse(res_area.approver);

    for (const value of arr_approver) {
      const query = `
        INSERT INTO dbo.${process.env.TABLE_LK_APPR} (id_lk, id_approver, approved)
        VALUES ('${id_lk}', '${value}', '${approved}')
        `;
    ...
    res.json({ ok: true, message: "approval inserted" });
  } catch (err) {
    console.error(err);
    try {
      await transaction.rollback();
    ...
    if (err.name === "RequestError" && err.message.includes("Timeout")) {
      ...
      res.status(500).json({
        ok: false,
        message: "Failed to create approval due to a server error.",
        error: err.message,
      });
    }
  }
...
app.post("/api/export-data-teknisi", async (req, res) => {
  try {
    ...
    if (dari && sampai) {
      conditions.push(
        "FORMAT(lk.waktu_selesai, 'yyyy-MM-dd') BETWEEN @dari AND @sampai"
      );
      request.input("dari", dayjs(dari).format("YYYY-MM-DD"));
      request.input("sampai", dayjs(sampai).format("YYYY-MM-DD"));
    }
    ...
    if (kode_area) {
      conditions.push("area.id = @kode_area");
      request.input("kode_area", kode_area);
    }
    ...
    const whereClause =
      conditions.length > 0 ? `WHERE ${conditions.join(" AND ")}` : "";
    const query = `SELECT lk.* FROM dbo.${process.env.TABLE_LK} AS lk 
JOIN dbo.${process.env.TABLE_CUSTOMER} AS cus 
ON lk.no_cus = cus.no_cus 
JOIN dbo.${process.env.TABLE_AREA} AS area 
ON cus.kode_area = area.id 
JOIN dbo.${process.env.TABLE_USER} AS teknisi 
ON lk.id_teknisi = teknisi.id ${whereClause} ORDER BY lk.waktu_selesai ASC`;
...
app.post("/api/export-report-teknisi", async (req, res) => {
  try {
    ...
    //     const dataQuery = `SELECT teknisi.name AS teknisi FROM dbo.${process.env.TABLE_AREA} AS area JOIN dbo.${process.env.TABLE_USER} AS teknisi on area.id_teknisi = teknisi.id
    ...
    //   dataInfoVal = dataResult.recordset[0];
    // }
...
app.get("/api/get-approver", async (req, res) => {
  try {
    const result = await pool.query(
      `SELECT * FROM dbo.${process.env.TABLE_USER}  WHERE role LIKE '%8%'`
    );
    res.json(result.recordset);
  } catch (err) {
    res.status(500).json({ error: "Database error" });
...
app.get("/api/get-area-kode", async (req, res) => {
  try {
    const result = await pool.query(
      `SELECT id, kode_area, nama_area, groups FROM dbo.${process.env.TABLE_AREA}`
    );
    res.json(result.recordset);
  } catch (err) {
    res.status(500).json({ error: "Database error" });
...
app.post("/api/create-area", upload.none(), async (req, res) => {
  const { kode_area, groups, id_supervisor, id_approver } = req.body;
  const now = dayjs();
...
app.post("/api/edit-area", upload.none(), async (req, res) => {
  const { groups, id_supervisor, kode_area, id_approver } = req.body;
  const now = dayjs();

  if (!groups) {
    return res
      .status(400)
      .json({ ok: false, message: "Groups ID is required." });
  }
  ...
  try {
    ...
    // Get existing area IDs for the group from the database
    const existingAreasResult = await request.query(`
      SELECT id FROM dbo.${process.env.TABLE_AREA} WHERE groups = '${groups}'
    `);
    const existingAreaIds = new Set(
      existingAreasResult.recordset.map((item) => item.id.toString())
    );

    const newAreaIds = new Set(
      kodeAreas.filter((item) => item.id).map((item) => item.id.toString())
    );

    // Identify IDs to be deleted (present in DB but not in new data from form)
    const idsToDelete = [...existingAreaIds].filter(
      (id) => !newAreaIds.has(id)
    );

    // Handle updates and inserts
    for (const item of kodeAreas) {
      // Check if the item's ID exists in the set of IDs from the database
      if (item.id && existingAreaIds.has(item.id.toString())) {
        // This is an existing item, perform an UPDATE
        const updateQuery = `
          UPDATE dbo.${process.env.TABLE_AREA}
          SET 
            kode_area = '${item.kode_area}', 
        ...
        // This is a new item, perform an INSERT
        const insertQuery = `
          INSERT INTO dbo.${process.env.TABLE_AREA} (
            kode_area, 
            nama_area, 
            groups, 
            id_supervisor, 
    ...
    if (idsToDelete.length > 0) {
      const deleteQuery = `
        DELETE FROM dbo.${process.env.TABLE_AREA}
        WHERE id IN (${idsToDelete.map((id) => `'${id}'`).join(",")})
      `;
      await request.query(deleteQuery);
    }

    await transaction.commit();
    res.json({ ok: true, message: "Area updated successfully." });
  } catch (err) {
    console.error(err);
    try {
      await transaction.rollback();
    ...
    if (err.name === "RequestError" && err.message.includes("Timeout")) {
      ...
      res.status(500).json({
        ok: false,
        message: "Failed to update area due to a server error.",
        error: err.message,
      });
    }
  }
